...
```{r histogram of mean total steps}
...
**Mean of Total steps across all days as a singular value:** `r format(mean(as.numeric(steps_df$x[(steps_df$x[,2] != "NaN"), 2])), scientific = FALSE)`

** **

Code chunk for making a Histogram of the median total number of steps taken each day
```{r histogram of median total steps}
...
**Median of Total steps across all days as a singular value:** `r format(median(steps_df[, 2]), scientific = FALSE)`
...
Starting Minute in HH:MM format: `r as.character(steps_df$date_time[which.max(steps_df$avg_steps)], "%H:%M")`
 
** **

####Total number of rows with missing values (coded as NA)
Count of Observations: `r nrow(actvt_df[(is.na(actvt_df$steps)), ])`
...
**Mean of Total steps across all days as a singular value:** `r format(mean(actvt_df$imputed_steps), scientific = FALSE)`

** **

**Median of Total steps across all days as a singular value:** `r format(median(actvt_df[(actvt_df$imputed_steps > 0), "imputed_steps"]), scientific = FALSE)`
...
```{r plot weekday-weekend activity}
library(lattice)
